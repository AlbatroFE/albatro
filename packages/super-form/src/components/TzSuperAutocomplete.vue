...
import TzRequest from "packages/utils/extension/TzRequest";
import { TzMessageConst } from '~/utils/extension/TzCommon';
Vue.use(Button);
...
export default class TzSuperAutocomplete extends Vue {
  ...
  createFilter(queryString: any) {
    return (data: any) => {
      return (
        ...
        (data.ext &&
          data.ext.toLowerCase().indexOf(queryString.toLowerCase()) === 0)
      );
    };
  }
  ...
  querySearch(queryString: any, cb: any) {
    var options =
      this.desc.options && this.desc.options.length ? this.desc.options : [];
  ...
  querySearchAsync(queryString: any, cb: any) {
    if (this.desc.options) {
      if (Array.isArray(this.desc.options)) {
        ...
        var request =
          this.desc.options.schema_meta_key.indexOf("Enum") > -1
            ? { key: this.desc.options.schema_meta_key }
            : TzRequest.onRequest(this.desc.options.schema, queryString);
        TzFetch.Post(this.desc.options.remote, request, false)
          .then((data: any) => {
            if (data && data.Data && data.Data.length) {
              var results = data.Data.map((x: any) => {
                return {
                  value: x[map["value"]],
                  ext: x[map["ext"]],
                  key: x[map["key"]]
