  ...
  <div class="al-super-form">
    ...
    >
      ...
      <div class="al-super-form-single" v-if="isGroupalbe">
        <el-row :gutter="24">
          <el-col
            :span="field.cols ? 8 * field.cols : 8"
            v-for="field of formDesc"
            :key="field.key"
          >
            <el-form-item
      ...
      <el-row class="al-super-form-command">
        <el-form-item class="al-super-form-command__append">
          ...
          <el-button @click="goBack" v-if="showBackBtn">
            {{ backBtnText ? backBtnText : "返回" }}
          </el-button>
        </el-form-item>
      </el-row>
    </el-form>
...
export default class AlSuperForm extends Vue {
  ...
  handleValidateForm(e: any) {
    if (this.rules) {
      (this.$refs["form"] as any).validate(
        (valid: boolean, invalidFields: any) => {
          if (valid) {
  ...
  async handleSubmitForm() {
    ...
    for (const field in data) {
      if (
        this.formDesc[field as any] &&
        this.formDesc[field as any].valueFormatter
      ) {
